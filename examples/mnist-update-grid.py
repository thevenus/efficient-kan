...
gridupdate_samples = DataLoader(trainset, batch_size=10000, shuffle=False, )

# Define model
zero_func = lambda x: x*0
model = KAN([28 * 28, 6, 6, 10], grid_eps=1, base_activation=zero_func, grid_size=5, spline_order=1)
...
for epoch in range(25):
    ...
    if (epoch == grid_update_freq):
        images, labels = next(iter(gridupdate_samples))
        images = images.view(-1, 28 * 28).to(device)
        print(images.shape)
        model.update_grid_from_samples(images, margin=0.1)
...
# with torch.no_grad():
#     for images, labels in gridupdate_samples:
#         images = images.view(-1, 28 * 28).to(device)
#         output = model(images)
#         val_loss += criterion(output, labels.to(device)).item()
#         val_accuracy += (
...
# val_accuracy /= len(valloader)
# print(
#     f"Model after grid update: Val Loss: {val_loss}, Val Accuracy: {val_accuracy}"
# )
